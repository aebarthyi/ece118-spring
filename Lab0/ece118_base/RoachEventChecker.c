 ...
 * MODULE #DEFINES                                                             *
 ******************************************************************************/
#define LIGHT_THRESHOLD 175 //Change numbers as needed
#define BUMPER_THRESHOLD 170 //Change as needed
...
uint8_t CheckLight(void) {
    static ES_EventTyp_t lastEvent = ROACH_ON;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t LIGHTLEVEL = AD_ReadADPin(LIGHTLEVEL); // change to read light level

    if (LIGHTLEVEL < LIGHT_THRESHOLD) { // check is light level under?
        curEvent = ROACH_OFF;
    } else {
        curEvent = ROACH_ON;
    }
    if (curEvent != lastEvent) { // check for change from last time
        ...
        SimpleRoachService(thisEvent); // Change it to your target service's post function
#else
        SaveEvent(thisEvent);
#endif   
    }
